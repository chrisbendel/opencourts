...
| DB client | [Kysely](https://kysely.dev) + [`kysely-d1`](https://github.com/aidenwallis/kysely-d1) dialect |
| DB types | `src/db/schema.ts`, auto-generated by `kysely-codegen` |
| Migrations | Plain `.sql` files in `migrations/`, applied via `wrangler d1 migrations apply` |
...
├── db/                      ← DB plumbing — only place that builds the Kysely instance
│   ├── client.ts            ← makeDb(env.DB) factory + CamelCasePlugin
│   └── schema.ts            ← Database interface, AUTO-GENERATED, do not edit
...
scripts/
└── codegen.sh               ← finds local D1 SQLite, runs kysely-codegen → src/db/schema.ts
...
## Kysely guardrails (read before touching `src/db/` or `migrations/`)

These exist because Kysely codebases rot in predictable ways. Don't skip.

### Boundaries

- **Only `src/db/client.ts` constructs the Kysely instance** via `makeDb(env.DB)`. The D1Dialect import lives only here.
- **Only `src/server/*.ts` calls `makeDb()`** to get a per-request Kysely instance. Routes never query the DB directly.
- **`src/db/schema.ts` is auto-generated by `kysely-codegen`.** Do not hand-edit. The header comment warns you. CI fails on diff.
- **Kysely is per-request, not module-scoped.** D1's `env.DB` binding is request-scoped, so the Kysely instance must be built inside server function handlers — no top-level singletons.

### Migrations

- Plain SQL files: `migrations/NNNN_description.sql`.
- Created via `pnpm db:new <name>` (which calls `wrangler d1 migrations create opencourts <name>` — wrangler picks the next number).
...
- Why SQL not Kysely TS migrations: D1's binding is only available inside a Worker. CLI-driven Kysely `Migrator` can't reach it. Wrangler's SQL-file workflow is the supported path.

### Workflow when changing the schema

```
1. pnpm db:new <name>                    → creates migrations/000N_<name>.sql (empty)
2. Edit the new file: write CREATE/ALTER SQL
3. pnpm db:migrate                       → applies to local D1 (.wrangler/state)
4. pnpm db:codegen                       → regenerates src/db/schema.ts from local D1
5. Commit the migration AND schema.ts together — same PR
6. After merge, before/after deploy: pnpm db:migrate:prod
```

Skipping step 4 = CI failure (CI re-runs codegen and `git diff --exit-code`s).
...
- Or write a new forward migration that undoes the previous change.
